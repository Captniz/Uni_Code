...
int main(int argc, char *argv[])
{
    ...
    int giper = Monitora(p, mese, ipo, iper, gipo, media, devstd);

    // stampa risultati
    cout << "Il numero di giorni di iperglicemia e': " << giper << endl
         << "il numero di giorni di ipoglicemia e': " << gipo << endl
         << "la media e': " << media << endl
         << "la deviazione standard e': " << devstd << endl;

    return 0;
}

// funzione Monitora
int Monitora(Paziente p, int n, int ipo, int iper, int &gipo, double &media, double &devstd)
{
    ...
    devstd = sqrt((somma - n_media_quadro) / colonne);
